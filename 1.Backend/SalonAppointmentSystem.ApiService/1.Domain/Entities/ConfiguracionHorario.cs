...
/// <summary>
/// Define los horarios de atención de la barbería
/// </summary>
public class ConfiguracionHorario : BaseEntity
{
    ...
    // Métodos de dominio
    /// <summary>
    /// Verifica si el horario es válido (HoraFin > HoraInicio)
    /// </summary>
    ...
    /// </summary>
    public bool EstaEnHorario(TimeSpan hora) => hora >= HoraInicio && hora < HoraFin;
    ...
    public bool AplicaParaFecha(DateTime fecha)
    {
        ...
        // Si no tiene fechas de vigencia, aplica siempre
        if (!FechaVigenciaDesde.HasValue && !FechaVigenciaHasta.HasValue)
            return true;

        // Verificar rango de vigencia
        var fechaSolo = fecha.Date;
        if (FechaVigenciaDesde.HasValue && fechaSolo < FechaVigenciaDesde.Value.Date)
            return false;
